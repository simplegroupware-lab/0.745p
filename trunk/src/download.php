...
if (isset($_REQUEST["item"]) and $_REQUEST["item"]=="session") {
  ...
  $path = dirname($path);
  $local_file = SIMPLE_CACHE."/upload/".$_SESSION["username"].md5($path)."--".urlencode($filename);
  //sha1($path)."--".urlencode($filename);
  if (file_exists($local_file)) _download_file($local_file,basename($local_file),"attachment");
 
  $local_file = SIMPLE_CACHE."/upload/".$_SESSION["username"].md5($path)."--".urlencode($filename).".link";
  //sha1($path)."--".urlencode($filename).".link";
...
function _download_resize($row_filename) {
  $row_filename_resize = SIMPLE_CACHE."/thumbs/".md5($row_filename)."_".filemtime($row_filename)."_".$_REQUEST["image_width"]."_".$_REQUEST["image_height"].".jpg";
  //sha1($row_filename)."_".filemtime($row_filename)."_".$_REQUEST["image_width"]."_".$_REQUEST["image_height"].".jpg";
