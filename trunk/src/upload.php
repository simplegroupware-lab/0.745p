...
if ($_REQUEST["item"]=="session") {
  ...
  if (sys_strbegins($filename,"~") or sys_strbegins($filename,".") or modify::getfileext($filename)=="tmp") {
    $target = SIMPLE_CACHE."/upload/".$_SESSION["username"].md5($path)."--".urlencode($filename);
	...
	$target_lnk = SIMPLE_CACHE."/upload/".$_SESSION["username"].md5($path)."--".urlencode($filename).".link";
...
if ($row_filename=="") {
  ...
  if (file_exists($row_filename.".lck") and !sys_can_unlock($row_filename,$_SESSION["username"])) {
	...
	if (strtolower($_REQUEST["action"])=="move" and !empty($_REQUEST["filename"])) {
	  ...
	  $tmpfile = SIMPLE_CACHE."/upload/".$_SESSION["username"].md5(dirname($path))."--".urlencode(basename($path));
	  //sha1(dirname($path))."--".urlencode(basename($path));
