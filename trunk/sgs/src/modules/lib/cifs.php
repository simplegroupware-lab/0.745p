  ...
  if (count($rows)>0) {
    if (in_array("filedata",$fields)) {
      foreach ($rows as $key=>$row) {
		...
		// Patch by Vlad Zaritsky 2014-11-27
		// Back urldecode cache record filename for allow long names to store
		// otherwice a long filename non ASCII, urlcoded can not to store with message 'Too long name of file'

		// Need to be tested for Windows server and how cache to be cleaned

		$filename = urldecode( sys_cache_get_file("cifs", $row["id"].$row["_lastmodified"], "--".modify::basename($row["id"]), true) );
